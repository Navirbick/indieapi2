...
router.post('/login', (req, res, next) =>{
    mysql.getConnection((error,conn) =>{
        ...
        conn.query(query, [req.body.email], (error, results, fields) =>{
            ...
            if(error) {return res.status(500).send({error: error})};
            if(results.length < 1) {
                return res.status(401).send({mensagem: 'falha na autenticação, erro 500'});
            }
           bcrypt.compare(req.body.senha, results[0].usu_senha, (err, result) => {
            if(err){
                return res.status(401).send({mensagem: 'falha na autenticação, tente novamente'});
            }
            if(result){
            ...
            return res.status(401).send({mensagem : 'falha na autenticação, erro 401'})
